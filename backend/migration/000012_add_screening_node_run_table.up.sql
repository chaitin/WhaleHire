...
ADD CONSTRAINT fk_screening_node_runs_task_id 
FOREIGN KEY (task_id) REFERENCES screening_tasks(id) ON DELETE CASCADE;

ALTER TABLE screening_node_runs 
ADD CONSTRAINT fk_screening_node_runs_task_resume_id 
FOREIGN KEY (task_resume_id) REFERENCES screening_task_resumes(id) ON DELETE CASCADE;
...
-- 添加状态检查约束
ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_status 
CHECK (status IN ('pending', 'running', 'completed', 'failed'));

-- 添加 attempt_no 检查约束
ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_attempt_no 
CHECK (attempt_no > 0);

-- 添加 tokens 检查约束
ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_tokens_input 
CHECK (tokens_input >= 0);

ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_tokens_output 
CHECK (tokens_output >= 0);

-- 添加 total_cost 检查约束
ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_total_cost 
CHECK (total_cost >= 0);

-- 添加 duration_ms 检查约束
ALTER TABLE screening_node_runs 
ADD CONSTRAINT chk_screening_node_runs_duration_ms 
CHECK (duration_ms >= 0);
