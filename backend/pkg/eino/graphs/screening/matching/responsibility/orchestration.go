...
type ResponsibilityAgent struct {
	chain *compose.Chain[*types.ResponsibilityData, *types.ResponsibilityMatchDetail]
}

// 输入处理，将ResponsibilityData转换为模板变量
func newInputLambda(ctx context.Context, input *types.ResponsibilityData, opts ...any) (map[string]any, error) {
	// 验证输入数据
	if input == nil {
...
// 输出处理，将模型输出解析为结构化结果
func newOutputLambda(ctx context.Context, msg *schema.Message, opts ...any) (*types.ResponsibilityMatchDetail, error) {

	if msg == nil || msg.Content == "" {
		return nil, fmt.Errorf("empty model output")
	}

	var output types.ResponsibilityMatchDetail
...
func NewResponsibilityAgent(ctx context.Context, llm model.ToolCallingChatModel) (*ResponsibilityAgent, error) {
	...
	// 构建处理链
	chain := compose.NewChain[*types.ResponsibilityData, *types.ResponsibilityMatchDetail]()
	chain.
...
// GetChain 获取处理链
func (a *ResponsibilityAgent) GetChain() *compose.Chain[*types.ResponsibilityData, *types.ResponsibilityMatchDetail] {
	return a.chain
}

// Compile 编译链为可执行的Runnable
func (a *ResponsibilityAgent) Compile(ctx context.Context) (compose.Runnable[*types.ResponsibilityData, *types.ResponsibilityMatchDetail], error) {
...
// Process 处理职责匹配
func (a *ResponsibilityAgent) Process(ctx context.Context, input *types.ResponsibilityData) (*types.ResponsibilityMatchDetail, error) {
	// 验证输入
...
func (a *ResponsibilityAgent) validateInput(input *types.ResponsibilityData) error {
	if input == nil {
...
func (a *ResponsibilityAgent) validateOutput(output *types.ResponsibilityMatchDetail) error {
	if output == nil {
	...
	for _, matchedResp := range output.MatchedResponsibilities {
		...
		// 验证LLM分析结果
		if matchedResp.LLMAnalysis != nil {
			if matchedResp.LLMAnalysis.MatchPercentage < 0 || matchedResp.LLMAnalysis.MatchPercentage > 100 {
				return fmt.Errorf("LLM match percentage must be between 0 and 100, got %f", matchedResp.LLMAnalysis.MatchPercentage)
			}
		}
	}
