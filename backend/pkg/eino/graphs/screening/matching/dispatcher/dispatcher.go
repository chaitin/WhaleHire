...
func NewDispatchData(input *types.MatchInput) DispatchData {
	...
	// 基本信息 Agent 数据
	data[types.BasicInfoAgent] = &types.BasicInfoData{
		JobProfile: input.JobProfile,
		Resume:     input.Resume,
	}

	// 技能 Agent 数据
	data[types.SkillAgent] = &types.SkillData{
	...
	// 职责 Agent 数据
	data[types.ResponsibilityAgent] = &types.ResponsibilityData{
	...
	// 经验 Agent 数据
	data[types.ExperienceAgent] = &types.ExperienceData{
...
func (d *Dispatcher) Process(ctx context.Context, input *types.MatchInput) (map[string]any, error) {
	// 验证输入数据
	if input == nil {
