...
- rationale 字段：包含 2-4 条简短说明（中文），阐述该评估角度的权重分配依据

**重要：rationale 字段的说明要求：**
- **不要**包含具体的权重数值（如"0.30"、"25%"等）
- **重点阐述**为什么某个维度应该给予较高或较低权重的依据和理由
- 说明应该聚焦于岗位特征、评估角度的逻辑，而不是数值本身
...
- rationale 字段的说明中**不要包含具体权重数值**，只阐述权重分配的依据和理由
`
