...
### 4. 到岗意愿与可用性 (availability)
- 就业状态为“离职/求职中”且期望城市与岗位地点高度一致：90-100分
- 就业状态为“在职”但期望城市一致，或可接受外地机会：70-89分
- 就业状态变量大、或期望城市与岗位地点存在差距：40-69分
- 未提供关键信息或明显不匹配：0-39分
...

{
  ...
  "sub_scores": {
    ...
    "availability": 到岗意愿与可用性分数（0-100的浮点数）
  },
  "evidence": [
    "包含关键信息的理由说明"
  ],
...
注意：
1. 所有分数必须是0-100之间的数值
2. 总分 = location*0.4 + salary*0.3 + department*0.2 + availability*0.1
3. evidence用于列出支撑评分的关键信息，每条不超过60个汉字，如信息缺失需指出
4. notes字段用于给出综合结论、风险提示或补充说明，需重点说明就业状态、期望城市与个人简介等信息`
...
- resume: 候选人姓名、年龄、当前城市、期望城市、工作年限、就业状态、期望薪资（包含解析后的区间与原文）、个人总结、荣誉奖项、近期经历摘要等基础信息
...
4. **到岗意愿与可用性**：结合就业状态、期望城市、个人总结、荣誉证书等信息，评估候选人到岗速度与稳定性
5. **证据与说明**：在evidence字段中列出关键事实支撑评分，在notes字段中总结总体结论、信息缺口与风险提示
