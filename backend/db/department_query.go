...
type DepartmentQuery struct {
	...
	withPositions *JobPositionQuery
...
func (dq *DepartmentQuery) QueryPositions() *JobPositionQuery {
	query := (&JobPositionClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) WithPositions(opts ...func(*JobPositionQuery)) *DepartmentQuery {
	query := (&JobPositionClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withPositions; query != nil {
		if err := dq.loadPositions(ctx, query, nodes,
			func(n *Department) { n.Edges.Positions = []*JobPosition{} },
			func(n *Department, e *JobPosition) { n.Edges.Positions = append(n.Edges.Positions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadPositions(ctx context.Context, query *JobPositionQuery, nodes []*Department, init func(*Department), assign func(*Department, *JobPosition)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(jobposition.FieldDepartmentID)
	}
