...
export function MatchingProcessModal({
  ...
  const [logs, setLogs] = useState<LogEntry[]>([
    { time: new Date().toLocaleTimeString('zh-CN', { hour12: false }), message: '系统初始化完成' },
    { time: new Date(Date.now() + 1000).toLocaleTimeString('zh-CN', { hour12: false }), message: '等待开始匹配任务...' },
    { time: new Date(Date.now() + 2000).toLocaleTimeString('zh-CN', { hour12: false }), message: '提示：点击"开始匹配"按钮开始处理' },
  ]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        const newProcessed = Math.floor((newProgress / 100) * selectedResumeCount);
        ...
        // 更新权重进度
        const currentWeightIndex = Math.floor((newProgress / 100) * weights.length);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <div className="overflow-y-auto px-6 py-6" style={{ maxHeight: 'calc(90vh - 180px)' }}>
          <div className="space-y-6">
            {/* 流程步骤指示器 - 横排展示 */}
            <div>
              <h3 className="mb-4 text-base font-semibold text-gray-900">匹配任务创建流程</h3>
              <div className="relative">
                ...
                <div className="flex items-start justify-between">
                  {[
                    ...
                    <div key={index} className="relative flex flex-col items-center" style={{ width: '18%' }}>
                      <div
                        className={cn(
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-xl bg-white p-6 shadow-sm">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-900">匹配进度</span>
                ...
                {/* 权重处理进度 */}
                <div className="rounded-xl bg-white p-6 shadow-sm">
                  <h4 className="mb-4 text-base font-semibold text-gray-900">权重处理进度</h4>
                  ...
                  <div className="space-y-3">
                    {weights.map((weight, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className={cn('flex h-8 w-8 items-center justify-center rounded-full', style.iconBg, 'bg-opacity-60')}>
                          ...
                          <div>
                            <span className={cn('text-xs font-medium', style.statusColor)}>
                              {style.statusLabel}
                            </span>
                          </div>
                        </div>
                ...
                {/* 实时处理日志 */}
                <div className="rounded-xl bg-white p-6 shadow-sm">
                  <h4 className="mb-4 text-base font-semibold text-gray-900">实时处理日志</h4>
                  <div
                    ref={logContainerRef}
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmPrevious}>确认</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
