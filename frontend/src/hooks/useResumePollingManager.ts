...
class ResumePollingManager {
  ...
  // 添加轮询项
  addPolling(resumeId: string, callback: (progress: ResumeParseProgress) => void) {
    ...
    console.log(`添加轮询: ${resumeId}, 当前轮询数量: ${this.pollingItems.size}`);
  }

  // 移除轮询项
  removePolling(resumeId: string, callback: (progress: ResumeParseProgress) => void) {
    const item = this.pollingItems.get(resumeId);
    if (!item) return;
    ...
    if (item.callbacks.size === 0) {
      ...
      console.log(`移除轮询: ${resumeId}, 剩余轮询数量: ${this.pollingItems.size}`);
    }
  ...
  private async pollAllItems() {
    const promises = Array.from(this.pollingItems.entries()).map(async ([resumeId, item]) => {
      try {
...
export const useResumePollingManager = () => {
  const callbackRef = useRef<((progress: ResumeParseProgress) => void) | null>(null);
  const currentResumeIdRef = useRef<string | null>(null);

  const startPolling = useCallback((resumeId: string, callback: (progress: ResumeParseProgress) => void) => {
    // 清理之前的轮询
    if (currentResumeIdRef.current && callbackRef.current) {
      globalPollingManager.removePolling(currentResumeIdRef.current, callbackRef.current);
    }
